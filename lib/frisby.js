...
var _hasHeader = function (headername, headers) {
    ...
    var lowerNames = headerNames.map(function (name) {
...
Frisby.prototype.expectJSONTypes = function (/* [tree], jsonTest */) {
    ...
    this.current.expects.push(function () {
        ...
        if ("*" === type || "&" === type) {
            ...
            for (var i = 0; i < itemCount; i++) {
                var errors = _jsonContainsTypes(jsonBody[i], jsonTest, self.current.isNot);
                if (errors.length == 0) {
                    match = true;
                    break;
                }
...
Frisby.prototype.expectJSON = function (jsonTest) {
    ...
    this.current.expects.push(function () {
        ...
        if ("*" === type || "&" === type) {
            ...
            let errors;
            for (var i = 0; i < itemCount; i++) {
                errors = _jsonContains(jsonBody[i], jsonTest, self.current.isNot);
                if (errors.length == 0) {
                    match = true;
                }
            }
            ...
            // 数组中数值的判断，用于对array中某个具体的元素进行对比 用法data.-1 data.0 -1表示最后一个元素
        } else if (typeof(type)==='number') {

            expect(jsonBody[type]).toContainJson(jsonTest, self.current.isNot);
        ...
        _withPath.call(self, path, jsonBody, function (jsonChunk) {
            ...
            if (res.valid === true) {
                ...
                throw new Error("JSONSchema validation failed with the following errors: \n\t> " +
                    res.errors.map(function (err, field) {
                        return err.stack.replace('instance', path);
                    }).join("\n\t> ")
                );
            }
        });
...
Frisby.prototype.toss = function (retry) {
    ...
    describe('[' + self.suiteName + '] - [' + self.name + ']', function () {
...
jasmine.Matchers.prototype.toContainJson = function (expected, isNot) {
    ...
    let errors = _jsonContains(this.actual, expected);
    this.handleErrors(errors, expected, isNot);
    return true;
};
jasmine.Matchers.prototype.toContainJsonTypes = function (expected, isNot) {
    ...
    // Way of allowing custom failure message - by throwing exceptions in utility function
    let errors = _jsonContainsTypes(this.actual, expected);
    this.handleErrors(errors, expected, isNot);
    return true;
};

jasmine.Matchers.prototype.handleErrors = function (errors, expected, isNot) {
    if (errors.length > 0) {
        if (!this.isNot && !isNot) {
            var msg = 'Expected: ' + JSON.stringify(expected);
...
function _jsonContains(jsonBody, jsonTest) {
    ...
    if (typeof jsonTest !== "object") {
        errors.push("Expected valid JavaScript object to be given, got " + typeof expected);
    }
    ...
    if (aType !== eType) {
        errors.push("Expected '" + aType + "' to be type '" + eType + "' for comparison");
    }
    ...
    for (key in jsonTest) {
        ...
        if (jsonBody) {
            ...
            if (kt === "object") {
                // NESTED expectJSON
                errors.push.apply(errors, _jsonContains(jsonBody[key], jsonTest[key]));
                continue;
            } else if (kt === "function") {
                ...
                if (_toType(keyType) === "object" && arguments.callee.caller.name === "") {
                    ...
                    if (typeof res === "boolean") {
                        if (true !== res) {
                            errors.push("Expected callback function on key '" + key + "' to return true");
                        }
                    }
                ...
                if (!test) {
                    errors.push("Expected " + _toType(jsonBody[key]) + " '" + jsonBody[key] + "' to match " + _toType(jsonTest[key]) + " '" + jsonTest[key] + "' on key '" + key + "'");
                }
            }
    ...
    if (errorKeys.length > 0) {
        errors.push("Keys ['" + errorKeys.join("', '") + "'] not present in JSON Response body");
    }
...
function _jsonContainsTypes(jsonBody, jsonTest) {
    ...
    for (key in jsonTest) {
        ...
        if (kt === "function") {
            ...
            // NESTED expectJSON
            errors.push.apply(errors, _jsonContainsTypes(jsonBody[key], jsonTest[key]));
            continue;
        }
        ...
        if (aType !== eType) {
            errors.push("Expected '" + aType + "' to be type '" + eType + "' on key '" + key + "'");
        }
